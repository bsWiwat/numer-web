...
import "./compositeTrapezoidal.css";
...
export default function ComTrapezoidal() {
  ...
  const [numberIntervals, setNumberIntervals] = useState<number>();
  ...
  const calculate = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:5006/api/Integrate/composite-trapezoidal",
        {
          method: "POST",
          headers: {
  ...
  const clearInputs = () => {
    ...
    setXEnd(undefined);
    setXStart(undefined);
    setNumberIntervals(undefined);
    setResult(null);
  };

  return (
    <div className="container">
      <h1>Composite Trapezoidal Integration</h1>
      <div className="input-group">
        <div className="function-input">
          ...
          >
            ...
            <TextField
              id="outlined-number"
              label="Number intervals"
              type="number"
              placeholder="4"
              sx={{ m: 0.5 }}
              value={numberIntervals ?? ""}
              onChange={(e) => setNumberIntervals(Number(e.target.value))}
            />
          </Box>
        </div>
