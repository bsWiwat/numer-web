...
export default function GaussSeidel() {
  ...
  const handleXInputChange = (index: number, value: number) => {
    const updatedMatrixX = [...initialX];
    updatedMatrixX[index] = value;
    setInitialX(updatedMatrixX);
  };
  ...
  const calculateRoot = async () => {
    ...
    try {
      const response = await fetch( "http://localhost:5006/api/linear/gauss-seidel",
        {
          method: "POST",
          headers: {
          ...
          {Array.from({ length: selectedSize }).map((_, row) => (
            <Box key={row} sx={{ display: "flex", alignItems: "center" }}>
              ...
              <TextField
                ...
                sx={{ m: 1, width: "6ch" }}
                onChange={(e) =>
                  handleXInputChange(row, Number(e.target.value))
                }
              />
            </Box>
          ))}
